...
DownloadList::receive_view_input(Input type) {
  ...
  input->bindings()[KEY_NENTER] = std::bind(&DownloadList::receive_exit_input, this, type);       // Numlock Enter
...
DownloadList::setup_keys() {
  ...
  m_bindings[KEY_NENTER]    = std::bind(&DownloadList::receive_view_input, this, INPUT_LOAD_MODIFIED);
